package kdx.graphics

import kotlin.math.*
import kdx.utils.NumberUtils
...
class Color {
    ...
    public override fun toString():String {
        var r = (255 * r).toInt().toString(16)
        var g = (255 * g).toInt().toString(16)
        var b = (255 * b).toInt().toString(16)
        var a = (255 * a).toInt().toString(16)
        if(r.length == 1) r = "0$r"
        if(g.length == 1) g = "0$g"
        if(b.length == 1) b = "0$b"
        if(a.length == 1) a = "0$a"
    ...
    companion object {
        ...
        {
            var hex = if (hex.get(0) == '#') hex.substring(1) else hex
            val r = hex.substring(0, 2).toInt(16)
            val g = hex.substring(2, 4).toInt(16)
            val b = hex.substring(4, 6).toInt(16)
            val a = if (hex.length != 8) 255 else hex.substring(6, 8).toInt(16)
        ...
        fun toFloatBits(r:Int, g:Int, b:Int, a:Int):Float {
            ...
            val floatColor = NumberUtils.intToFloatColor(color)
            return floatColor
        }
        /** Packs the color components into a 32-bit integer with the format ABGR and then converts it to a float.
         * @return the packed color as a 32-bit float
